# ラグナロク・ミレニアム（Ragnarok Millennium）

ストア表示名は「ラグナロク・ミレニアム」。テーマは西暦2000年・神と悪魔の戦い。
数字カード1〜9は大魔王→…→人間の戦士→…→神の9階級、昼は天が・夜は魔が優勢で、
革命でその昼夜が反転します。

このリポジトリは、Android先行のオリジナルカードゲームを開発するためのモノレポです。
v1.0の対象は、Android 10以上のスマートフォン・タブレット、横画面、CPU戦と
2〜6人のフレンド対戦です。
...
- M1/M2/M3 完了。M4（オンラインフレンド対戦）実装中。
- 進捗の詳細は `docs/progress/` を参照。
