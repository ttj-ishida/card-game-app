-- M4-EX-05 follow-up: persist the round-level consecutive-pass streak.
--
-- submit-play rebuilds a RoundState from the DB on every request and never
-- passed the running consecutive-pass count through, so `buildServerRoundState`
-- always started it at 0. `evaluatePass`'s clearsField threshold
-- (consecutivePasses >= responderCount) was then only ever reachable when
-- there was exactly one responder — i.e. a 2-player round. In a 3+ player
-- round, an all-pass cycle could never clear the field.

alter table public.online_round_public_state
  add column if not exists consecutive_passes integer not null default 0;

alter table public.online_round_public_state
  add constraint online_round_public_state_consecutive_passes_check
  check (consecutive_passes >= 0);

comment on column public.online_round_public_state.consecutive_passes is
  '直近のリード以降、連続でパスした人数。全員パスでの場流し判定に使う（M4-EX-05 follow-up）。';
...
begin
  ...
  update public.online_round_public_state
     set state_version = next_version,
         ...
         consecutive_passes = coalesce((next_public_state->>'consecutive_passes')::integer, current_state.consecutive_passes),
         updated_at = now()
   where round_id = target_round_id;
