...
「編集警告（無効な場）」は場エディタ（`sandbox.field.edit`）で組み合わせとして無効なカード列を作った時に `sandbox.field.invalid` を表示し、「場に設定」ボタンを無効化することで実際に到達可能。

## 編集対象（spec §2.1 網羅）

昼夜 / 人数 / 手番 / 各プレイヤーの手札・status・スキル効果・スキル使用済みフラグ / 場（読み取り + カード列エディタ + 最終出し手 + 妥当時のみ `setFieldCards` でコミット）/ 属性ロック / 追加封印 / 連続パス / 捨て札（追加・削除）。

## 確認

| コマンド | 結果 |
| --- | --- |
| `npm run mobile:test` | PASS（50件） |
...
- 場のカード列は画面ローカルの `useState`（`NumberCard[]`）で編集し、`isValidFieldCards` が真のときだけ `setFieldCards` でストアへ反映する。無効な列はストアに出さず警告表示のみ。
