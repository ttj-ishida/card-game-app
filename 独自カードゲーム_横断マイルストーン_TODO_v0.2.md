...
- 各期間には20%程度の不確実性を含む。フレンド対戦の同期問題が大きい場合は追加バッファを確保する。
- v1.0はAndroid 10以上、CPU＋フレンド対戦、日本語のみ、全世界・13歳以上を対象とする。
- M5はv1.0対象外の将来版バックログであり、M6～M8の依存先にしない。

## 2. 全体マイルストーン

| ID | 目安 | 実現するもの | Supabase | Expo | グラフィック | リリース地点 |
|---|---|---|---|---|---|---|
| M0 | W1～2 | Android開発土台とカードカタログ | 開発環境、マイグレーション、マスタ | Android起動、画面遷移、環境切替 | アート方針、仮カード | 内部確認 |
| M1 | W3～6 | ローカル・ルールサンドボックス | ルール版・マスタ | 純粋ルールエンジン、デバッグ盤面 | 仮カード一式、状態アイコン | ルール検証版 |
| M2 | W7～12 | CPUと1局遊べる縦切り | 練習結果保存の土台 | 標準CPU、対局画面、結果・再戦 | 昼夜背景、代表カード、基本演出 | CPUプロトタイプ |
| M3 | W13～18 | 全ルール対応CPU版 | プロフィール、統計、イベント | 全スキル、履歴、チュートリアル | 9階級、スキル、効果素材 | CPUアルファ |
| M4 | W19～28 | 招待リンクによる2～6人フレンド対戦 | ルーム、Realtime、再接続、CPU引継ぎ | 招待、オンライン対局、同期 | オンライン状態・ルームUI | クローズドα |
| M6 | W29～34 | 製品品質のグラフィックとAndroid性能 | アセット版管理 | 横画面、画像最適化、軽量設定 | 全本番素材、演出最終化 | ビジュアルRC |
| M7 | W35～39 | 広告・分析・運用 | 同意、削除、分析、運用ログ | AdMob、Firebase、規約、障害表示 | ストア・同意・告知素材 | 収益化RC |
| M8 | W40～44 | Android Google Play公開版 | 本番環境、監視、バックアップ | Android実機E2E、ビルド、申請 | 最終QA、Google Play素材 | v1.0 |
| M5 | v1.0公開後 | 将来版：人間4人の公式対戦 | マッチング、公式結果、順位、不正防止 | 公式対戦、30秒、棄権、順位 | 公式対戦・結果・順位素材 | 将来β |
...
| G2 | M3完了 | CPU版の限定テスト |
| G3 | M4完了 | 招待制フレンド対戦α |
| G4 | M8完了 | Android版をGoogle Playで段階公開 |
| G5 | 将来M5完了 | 招待制公式対戦β |
...
| M5（将来） | MODE-OFFICIAL、MATCH、TIMER、TIMEOUT、RANKING、FAIR |
| M6 | UI、FX、I18N、NFR-P、アクセシビリティ |
| M7 | ADS、OPS、ACCOUNT、外部サービス、プライバシー |
| M8 | Android v1.0、NFR、テスト・受入条件、REL、Google Play公開 |
...
- 開発環境へ接続したExpoアプリがAndroidエミュレーター・Android実機で起動する。
...
| M0-EX-01 | EX | Expo＋TypeScriptのプロジェクトを作成する | M0-PM-01 | Androidエミュレーター・実機で初期画面が開く |
...
HTTPS招待リンクからルームへ参加し、人間2～6人が同じ対局状態を見ながら、配布から結果・再戦までオンラインで完走できる。未導入端末は案内ページからGoogle Playへ進める。

### 8.2 終了条件

- ルーム作成者が人数、手番時間、CPU引継ぎ設定を指定でき、退出時は最古参の残存参加者へ権限移譲される。
- HTTPS招待リンクから入室でき、未導入端末は案内ページからGoogle Playへ進める。
...
| M4-SB-12 | SB | フレンド対戦の退出・棄権・任意CPU引継ぎを作る | M4-SB-01、M2-EX-03 | 退出者を敗北として記録し、席を除外またはCPUへ渡せる |
| M4-EX-01 | EX | フレンドルーム作成画面を作る | M4-SB-04 | 人数・時間・CPU引継ぎを指定できる |
| M4-EX-02 | EX | HTTPS招待リンクと未導入案内ページによる参加画面を作る | M4-SB-04 | 導入済みなら入室し、未導入ならGoogle Playへ進める |
...
本マイルストーンはv1.0公開後の将来版バックログである。着手時点で要件・法令・運用費を再確認したうえで、人間4人固定の公式対戦を完成させる。
...
- Android 10以上のスマートフォン・タブレットを横画面で使用し、手札18枚でも操作不能にならない。
- 通常60fps、軽量設定30fps以上を満たす。
...
| M6-EX-03 | EX | Android横画面のスマホ・タブレット向け手札レイアウトを調整する | M4-EX-05、M4-EX-09 | 2～6人、6～18枚で操作できる |
| M6-EX-04 | EX | 演出短縮・軽量設定を完成する | M3-EX-08 | 重いフィルタや粒子を抑制できる |
| M6-EX-05 | EX | 将来英語化を想定した可変レイアウトを確認する | M0-EX-04 | 仮英語長文でも主要UIが欠けず、v1.0表示は日本語に限定される |
...
| TODO ID | 領域 | TODO | 依存 | 完了条件 |
|---|---|---|---|---|
| M7-PM-01 | 横断 | 毎局後広告、報酬スキップ権1回、買い切りなしを仕様化する | M4-QA-05 | 確定広告仕様が画面・データ・テストへ反映される |
| M7-PM-02 | 横断 | 全世界・13歳以上・日本語のみの地域別同意要件を整理する | M7-PM-01 | 地域別データ収集・同意一覧が完成する |
| M7-SB-01 | SB | 分析イベントの命名とスキーマを作る | M7-PM-02 | 版管理されたイベント一覧がある |
| M7-SB-02 | SB | 対局IDを使った障害・監査検索を整備する | M4-SB-03、M4-SB-06～08 | 1局の操作を時系列で追跡できる |
| M7-SB-03 | SB | 本人イベント削除と他参加者勝敗集計維持を作る | M7-PM-02 | 参照切れなく確定削除方針を実行できる |
| M7-SB-04 | SB | サービス停止・フレンド対戦受付停止フラグを作る | M4-SB-04 | 新規フレンド対戦受付を安全に止められる |
| M7-SB-05 | SB | エラー率、対局成立数、切断率の監視を作る | M7-SB-02 | 閾値超過を検知できる |
| M7-EX-01 | EX | 広告SDKと同意処理を組み込む | M7-PM-01、02 | 同意状態に応じて配信を制御できる |
| M7-EX-02 | EX | 毎局の結果後広告とスキップ権消費を実装する | M7-EX-01 | 対局中に表示せず、スキップ権を1回だけ消費する |
...
| M7-EX-06 | EX | 退会・本人イベント削除導線を作る | M7-SB-03 | 他参加者には勝敗集計だけ残ることを明示して要求できる |
...
本番Supabase、Android提出ビルド、監視、バックアップ、復旧手順、Google Play素材を揃え、CPU＋フレンド対戦の日本語v1.0を全世界・13歳以上向けに段階公開する。

### 12.2 終了条件

- v1.0 Must要件に対応する自動・手動テストが成功する。
- 重大度「高」の既知不具合が0件である。
- 本番マイグレーションと切戻しを検証済みである。
- CPU戦・フレンド対戦の勝敗集計の二重加算と非公開情報漏洩がない。
- Android 10以上のスマートフォン・タブレットで横画面ビルドが動作する。
- 同時接続100人、可用性99.0%、95%を2秒以内、クラッシュなし99.0%以上を確認する。
- RPO 24時間、RTO 8時間、ログ90日、月額上限1万円の運用手順がある。

### 12.3 TODO

| TODO ID | 領域 | TODO | 依存 | 完了条件 |
|---|---|---|---|---|
| M8-PM-01 | 横断 | Android v1.0のCPU＋フレンド範囲と対象外を最終確認する | M7-QA-03 | 公式、iOS、英語が対象外として凍結される |
| M8-PM-02 | 横断 | Android 10以上、全世界、13歳以上、無料＋広告を確認する | M8-PM-01 | Google Play入力情報が確定する |
...
| M8-SB-04 | SB | 本番監視、アラート、フレンド対戦受付停止を検証する | M7-SB-04、05 | テスト障害で通知・停止できる |
| M8-SB-05 | SB | 本番負荷を想定した同時対局テストを行う | M8-SB-02 | 確定した性能目標を満たす |
| M8-EX-01 | EX | 本番ビルド設定、署名、環境変数を確定する | M8-SB-01 | 開発・検証・本番を取り違えない |
| M8-EX-02 | EX | Androidスマホ・タブレット実機の全E2Eを実行する | M8-EX-01 | CPU、フレンド、広告、退会が完走する |
| M8-EX-03 | EX | Google Play審査用アカウントと確認手順を用意する | M8-EX-02 | 審査担当が主要機能を確認できる |
| M8-EX-04 | EX | クラッシュ、ANR、メモリ、通信失敗を最終確認する | M8-EX-02 | 確定した品質目標を満たす |
| M8-EX-05 | EX | Google Play段階公開ビルドを作成・提出する | M8-EX-02～04 | Google Play審査へ提出できる |
| M8-GR-01 | GR | Android用アイコン、起動画面、Google Play画像を完成する | M7-GR-02 | Google Playの必要サイズが揃う |
...
| M8-QA-02 | QA | CPU・フレンドの回帰、切断、競合、情報漏洩、広告失敗を再実行する | M8-QA-01 | リリース阻害不具合が0件 |
...
M5の公式対戦はv1.0公開後の独立バックログとする。本番グラフィック制作はM2からM4と並行するが、最終レイアウトを固定するM3より前に36枚を量産しない。
...
| 退会時のイベント削除と勝敗集計維持 | M4前 | 参照切れと他参加者統計の欠落を避けるため |
| 広告・対象年齢・地域別同意 | M7前 | Google Play・画面・プライバシーへ影響するため |
...
- M4の同期・再接続検証中に、36枚の書出しと演出素材を量産する。
...
| 領域 | 目安工数 | 主な不確実性 |
|---|---:|---|
| Supabase | 150～250時間 | サーバー権威、Realtime、再接続、RLS、削除・集計 |
| Expo | 300～480時間 | ルールUI、手札操作、同期、Android端末差、広告 |
| グラフィック | 250～500時間 | 9階級の描込み量、演出、本番書出し、修正回数 |
| 結合・QA・リリース | 150～250時間 | 通信障害、競合、Android実機、Google Play審査 |
| 合計 | 850～1,480時間 | 本番アート品質とフレンド同期不具合で変動 |

W40～44は下限寄りの目標である。週15～20時間の個人開発では、実際のカレンダーは約44～74週を見込む。将来M5の公式対戦は別途約10～16週を見込む。

## 16. 遅延時のスコープ削減順

品質やデータ整合性を落とさず、次の順で機能を後ろへ送る。

1. プッシュ通知をv1.0から外す（確定済み）。
2. カジュアル対戦と公式対戦をv1.0から外す（確定済み）。
3. CPU難易度を標準1種類にする（確定済み）。
4. プロフィール画像のカスタマイズを固定アイコンにする。
5. 演出差分と粒子量を減らし、ルール可読性を維持する。
6. 英語版は構造だけ確保し、翻訳公開を将来へ送る（確定済み）。
7. 報酬広告は、検証がリリースを阻害する場合のみ通常広告を維持して将来版へ送る。
