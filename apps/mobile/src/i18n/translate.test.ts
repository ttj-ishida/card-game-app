...
test('jaDictionary includes every invite-link key required by M4-EX-02', () => {
  const requiredKeys: TranslationKey[] = [
    ...
    'onlineRoom.join.getAppAndroid',
    'onlineRoom.join.getAppIos',
    'onlineRoom.join.iosComingSoon',
    'onlineRoom.join.playOnWeb',
    ...
    'onlineRoom.error.notHost',
    'onlineRoom.lobby.waitingHost',
  ];
  for (const key of requiredKeys) {
    assert.equal(typeof jaDictionary[key], 'string');
    assert.notEqual(jaDictionary[key].length, 0);
...
test('jaDictionary includes every online-room play screen key required by M4-EX-05', () => {
  const requiredKeys: TranslationKey[] = [
    ...
    'onlineRoom.play.opponentCpu',
    'onlineRoom.play.opponentLeft',
    'onlineRoom.play.eventTakeover',
    'onlineRoom.play.eventForfeit',
