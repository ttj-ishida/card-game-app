...
function suitColor(c: ThemeColors, suitCode: string): string {
  switch (suitCode) {
    case 'SUIT_FIRE':
      return c.suit.fire;
    case 'SUIT_WATER':
...
function CardChip({ card }: { card: NumberCard }) {
  ...
  return (
    <View
      ...
      style={[styles.chip, { borderColor: suitColor(c, card.suitCode) }]}
    >
            ...
            <View style={styles.row}>
              ...
              {draft.players.map((player) => (
                <Pressable
                  ...
                  onPress={() =>
                    state.editRound((round) => setActivePlayer(round, player.playerId))
                  }
                  style={styles.miniButton}
                >
                  ...
                  <View style={styles.statusWrap}>
                    {(['ACTIVE', 'PASSED', 'OUT'] as const).map((status) => (
                      <Pressable
                        ...
                        onPress={() =>
                          state.editRound((round) =>
                            setPlayerStatus(round, player.playerId, status),
                          )
                        }
                        style={styles.miniButton}
                      >
              ...
              <>
                <View style={styles.row}>
                  ...
                  <Pressable
                    ...
                    onPress={() =>
                      state.editRound((r) =>
                        setFieldCountLocked(r, !r.activeField!.lock.countLocked),
                      )
                    }
                ...
                <View style={styles.row}>
                  ...
                  <Pressable
                    ...
                    onPress={() =>
                      state.editRound((r) =>
                        setFieldSuitUniform(r, !r.activeField!.lock.suitUniform),
                      )
                    }
                      ...
                      <Pressable
                        ...
                        onPress={() => {
                          ...
                          const next = current.includes(suit)
                            ...
                            : [...current, suit];
                          state.editRound((r) =>
                            setFieldSuitFixed(r, next.length > 0 ? next : null),
                          );
                        }}
              ...
              {RANK_CODES.map((rankCode) =>
                SUIT_CODES.map((suitCode) => (
                  <Pressable
                    ...
                    onPress={() =>
                      state.editRound((round) =>
                        addCardToHand(round, addTarget, rankCode, suitCode),
                      )
                    }
                  >
                    <CardChip card={makeSandboxCard(rankCode, suitCode)} />
...
const makeStyles = (c: ThemeColors) =>
  StyleSheet.create({
    ...
    label: { fontSize: typography.size.caption, color: c.ink.secondary },
    ...
    playerRow: {
      ...
      gap: spacing.xs,
      borderTopWidth: 1,
      borderTopColor: c.surface.table.day,
    ...
    chipSuit: { fontSize: 10, color: c.ink.secondary },
    selectedChipWrap: {
      borderWidth: 2,
      borderColor: c.ink.primary,
      borderRadius: radius.control,
    ...
    miniButtonText: { fontSize: typography.size.caption, color: c.ink.primary },
    pill: {
      borderWidth: 1,
      borderColor: c.ink.primary,
      borderRadius: radius.card,
    ...
    pillText: { fontSize: typography.size.caption, color: c.ink.primary },
    runButton: {
